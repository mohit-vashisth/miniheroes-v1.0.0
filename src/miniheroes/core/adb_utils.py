...
def clear_app_data(device_id: str, package_name: str) -> None:
    try:
        ...
        logger.info("[CLEAR] %s %s", device_id, package_name)
    except Exception as exc:
        logger.error("[CLEAR ERROR] %s: %s", device_id, exc)
...
def wait_for_emulator_boot(port: int, max_attempts: int = 40, delay: int = 3) -> bool:
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if output == "1":
                logger.info("[BOOT] %s ready", device_id)
                return True
        except Exception:
            pass

        if attempt == 1 or attempt % 5 == 0:
            logger.info("[BOOT WAIT] %s attempt %s/%s", device_id, attempt, max_attempts)
        time.sleep(delay)

    logger.error("[BOOT TIMEOUT] %s", device_id)
    return False
...
def adb_tap(device_id: str, x: int, y: int, t: int, console_path: str) -> bool:
    logger.debug("[TAP] %s (%s,%s) wait=%ss", device_id, x, y, t)
    ...
    if emulator_index is not None:
        try:
            ...
            if dn_result.returncode == 0:
                logger.debug("[TAP] dnconsole success for %s", device_id)
                return True
        except Exception as exc:
            logger.debug("[TAP] dnconsole tap failed for %s: %s", device_id, exc)
    ...
    try:
        ...
        if adb_result.returncode == 0:
            logger.debug("[TAP] adb success for %s", device_id)
            return True
    except Exception as exc:
        logger.debug("[TAP] adb tap failed for %s: %s", device_id, exc)

    logger.error("[TAP FAILED] %s (%s,%s)", device_id, x, y)
    return False
